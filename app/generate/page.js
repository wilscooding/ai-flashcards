...
// 						color="primary"
// 						sx={{
// 							width: "300px", // Matches TextField
// 							height: "45px", // Slightly taller for button
// 							textTransform: "none",
// 							borderRadius: "8px",
// 							fontSize: "1rem",
// 							fontWeight: "bold",
...
// 					<Typography
// 						variant="h5"
// 						sx={{ color: "#000", mb: 2, align: "center" }}
// 					>
// 						Flashcards Preview
// 					</Typography>
...
export default function Generate() {
	...
	const [name, setName] = useState("");
	const [open, setOpen] = useState(false);
	const [difficulty, setDifficulty] = useState("Beginner"); // Default to Beginner
	...
	const handleSubmit = async () => {
		try {
			const response = await fetch("/api/generate", {
				method: "POST",
				body: JSON.stringify({ prompt: text, difficulty }), // Send difficulty level
			...
			>
				...
				>
					<TextField
						value={text}
						onChange={(e) => setText(e.target.value)}
						label="Enter a language or topic"
						variant="outlined"
						sx={{
							width: "300px",
							mb: 2,
